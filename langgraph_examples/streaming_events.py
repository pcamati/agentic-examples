...
async def stream_events(
    ...
    """
    Stream events of the graph execution.

    This function is similar to the .invoke method, returning the final
    event after reaching the END node.
    """
    async for event in graph.astream_events(
        ...
        if event_type == "on_chat_model_stream":
            data = event["data"]
            event_obj = data["chunk"]
            if event_obj and hasattr(event_obj, "content"):
...
async def run() -> None:
    ...
    last_event = await stream_events(initial_state, graph)
